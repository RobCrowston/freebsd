...
bcm_xhci_probe(device_t dev)
{
	...
	if (!fdt_is_compatible_strict(root, "raspberrypi,4-model-b"))
		return (ENXIO);
...
static void
bcm_xhci_check_xhci_firmware(device_t dev, bool expect_loaded)
{
	uint32_t xhci_rev;
	bool loaded;

	xhci_rev = pci_read_config(dev, VL805_FIRMWARE_REG, 4);
	loaded = !(xhci_rev == 0 || xhci_rev == 0xffffffff);

	if (expect_loaded && !loaded)
		device_printf(dev, "warning: xhci firmware not installed.\n");
	else if (bootverbose && !loaded)
		device_printf(dev, "note: xhci firmware not installed.\n");
	else if (bootverbose)
		device_printf(dev,
		    "note: xhci firmware installed; firmware is revision %x.\n",
		     xhci_rev);
}
...
{
	uint32_t xhci_dev_addr;
	int error;

	bcm_xhci_check_xhci_firmware(dev, false);
	...
	error = bcm2835_mbox_notify_xhci_reset(xhci_dev_addr);
	if (error)
		device_printf(dev,
	...
	DELAY(1000);
	bcm_xhci_check_xhci_firmware(dev, true);
...
DEFINE_CLASS_1(xhci, bcm_xhci_driver, bcm_xhci_methods,
